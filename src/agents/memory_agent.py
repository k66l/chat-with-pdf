...
class MemoryAgent:
    ...
    def __init__(self):
        ...
        self.default_recent_context = self._get_default_recent_context_size()
        self.default_max_sessions = self._get_default_max_sessions()
        self.summary_context_size = self._get_summary_context_size()
        logger.info("Memory Agent initialized", max_messages=self.max_messages)

    def _get_default_recent_context_size(self) -> int:
        """Get configurable default recent context size."""
        return getattr(settings, 'default_recent_context_size', 5)

    def _get_default_max_sessions(self) -> int:
        """Get configurable default maximum sessions."""
        return getattr(settings, 'default_max_sessions', 100)

    def _get_summary_context_size(self) -> int:
        """Get configurable summary context size."""
        return getattr(settings, 'summary_context_size', 3)
    ...
    def clear_session(self, session_id: str) -> bool:
        ...
        try:
            if session_id in self.sessions:
                ...
                logger.warning("Session not found for clearing",
                               session_id=session_id)
                return False
    ...
    def get_recent_context(self, session_id: str, max_messages: int = None) -> List[ChatMessage]:
        """Get recent messages for context."""
        try:
            if max_messages is None:
                max_messages = self.default_recent_context
    ...
    def cleanup_old_sessions(self, max_sessions: int = None) -> int:
    ...
    def get_conversation_summary(self, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            if len(user_messages) > 0:
                # Use configurable context size for recent topics
                recent_topics = user_messages[-self.summary_context_size:]
